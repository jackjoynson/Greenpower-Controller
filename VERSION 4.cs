...
namespace ControlPanel
{
    public partial class Form1 : Form
    {
        ...
        /* Initial parameters for AmpHours dial */
        double ampHoursLowRange = 0.0;
        double ampHoursHighRange = 40.0;
        double ampHoursInitValue = 20.0;

        /* Initial parameters for Temp dial */
        double tempLowRange = 0.0;
        double tempHighRange = 40.0;
        double tempInitValue = 20.0;

        //MY ADDED VARIABLES
        double currentValue = 0.0; //THIS NEEDS TO COME FROM ARDUINO
        double voltageValue = 0.0;
        double ampHoursValue = 0.0;
        ...
        private void glgControl1_GlgH( object sender, GenLogic.GlgEventArgs e )
        {
            ...
            e.viewport.SetDResource("AmpHoursDial/HandlerDisabled", 1.0);
            e.viewport.SetDResource("AmpHoursDial/Low", ampHoursLowRange);
            e.viewport.SetDResource("AmpHoursDial/High", ampHoursHighRange);
            e.viewport.SetDResource("AmpHoursDial/Value", ampHoursInitValue);

            e.viewport.SetDResource("TempDial/HandlerDisabled", 1.0);
            e.viewport.SetDResource("TempDial/Low", tempLowRange);
            e.viewport.SetDResource("TempDial/High", tempHighRange);
            e.viewport.SetDResource("TempDial/Value", tempInitValue);
